import {
  ...
  saveBtn,pauseBtn, pauseIcon, addBodyBtn, resetBtn, recenterBtn, 
...
function applySnap(state) {
  ...
  if (state.ui) {
    ...
    TIME_SCALE = state.ui.TIME_SCALE ?? TIME_SCALE;
...
function updatePhysics(dt) {
  ...
  for (const b of bodies) {
    ...
    if (b.image === SUN_IMAGE) {
      b.angularVelocity = Math.max( -MAX_SPIN_SUN, Math.min(MAX_SPIN_SUN, b.angularVelocity));
    ...
    b.angularVelocity = Math.max(-MAX_SPIN,Math.min(MAX_SPIN, b.angularVelocity));
